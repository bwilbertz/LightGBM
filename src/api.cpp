...
namespace LightGBM {
  ...
  Booster::Booster(const Dataset* train_data, const char* parameters) {
  ...
  void Booster::Init(const Dataset* train_data) {
    ...
    if (Network::num_machines() == 1
        && config_.boosting_config.tree_learner_type
            != std::string("serial")) {
      Log::Warning(
  ...
  void Booster::MergeFrom(const Booster* other) {
    std::lock_guard<std::mutex> lock(mutex_);
    boosting_->MergeFrom(other->boosting_.get());
  }

  void Booster::CreateObjectiveAndMetrics() {
  ...
  void Booster::ResetTrainingData(const Dataset* train_data) {
  ...
  void Booster::ResetConfig(const char* parameters) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto param = ConfigBase::Str2Map(parameters);
  ...
  void Booster::AddValidData(const Dataset* valid_data) {
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  void Booster::RollbackOneIter() {
    std::lock_guard<std::mutex> lock(mutex_);
    boosting_->RollbackOneIter();
  }

  void Booster::Predict(int num_iteration, int predict_type, int nrow,
                 std::function<std::vector<std::pair<int, double>>(int row_idx)> get_row_fun,
                 double* out_result, int64_t* out_len) {
    OverallConfig config;

    Predict(num_iteration, predict_type, nrow, get_row_fun, config.io_config, out_result, out_len);
  }
  ...
  void Booster::Predict(int num_iteration, int predict_type, const char* data_filename,
  ...
  void Booster::SetLeafValue(int tree_idx, int leaf_idx, double val) {
    std::lock_guard<std::mutex> lock(mutex_);
    dynamic_cast<GBDTBase*>(boosting_.get())->SetLeafValue(tree_idx, leaf_idx, val);
  }

  int Booster::GetEvalCounts() const {
    int ret = 0;
  ...
  int Booster::GetEvalNames(char** out_strs) const {
    int idx = 0;
    for (const auto& metric : train_metric_) {
  ...
  #pragma warning(disable : 4996)
  int Booster::GetFeatureNames(char** out_strs) const {
    int idx = 0;
...
Dataset* CreateDatasetFromMat(const void* data, int data_type, int32_t nrow,
  ...
  auto get_row_fun = RowFunctionFromDenseMatric(data, nrow, ncol, data_type,
                                                is_row_major);
  if (reference == nullptr) {
    // sample data first
    Random rand(config.io_config.data_random_seed);
    int sample_cnt = static_cast<int>(
        nrow < config.io_config.bin_construct_sample_cnt ?
            nrow : config.io_config.bin_construct_sample_cnt);
...
Dataset* CreateDatasetFromCSR(const void* indptr, int indptr_type,
...
Dataset* CreateDatasetFromCSC(const void* col_ptr, int col_ptr_type,
                              const int32_t* indices, const void* data,
